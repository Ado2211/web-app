...
export class ArticleService extends TypeOrmCrudService<Article> {
  ...
  async editFullArticle(articleId: number, data: EditArticleDto): Promise<Article | ApiResponse> {
    ...
    if (!savedArticle) {
      return new ApiResponse('error', -5001, 'Colud not save new article data!');
    }
    ...
    if (newPriceString !== lastPriceString) {
      ...
      if (!savedArticlePrice) {
        return new ApiResponse('error', -5001, 'Colud not save new article price!');
      }
    }
  ...
  async search(data: ArticleSearchDto): Promise<Article[]> {
    ...
    builder.innerJoinAndSelect("article.articlePrices", "ap",
      "ap.created_at = (SELECT MAX(ap.created_at) FROM article_price AS ap WHERE ap.article_id = article.article_id )");      //ovo nije primjer najbolje prakse
